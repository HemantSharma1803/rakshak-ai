...
function openModal(type){const m=$('#modalContent'); if(!state.last){m.innerHTML='<span class="section-kicker">NO ACTIVE ANALYSIS</span><h2>Run a threat analysis first.</h2><p style="color:#7891a4;line-height:1.6">Generate a result before creating a family warning or incident report.</p><button class="primary-btn" id="modalGoScan">Start analysis →</button>'; $('#modalBackdrop').hidden=false; $('#modalBackdrop').style.display='grid'; $('#modalGoScan').onclick=()=>{closeModal();route('scan')}; return;} if(type==='family')m.innerHTML=`<span class="section-kicker">FAMILY PROTECTION CARD</span><h2>Send a clear warning, not panic.</h2><div class="warning-card"><div class="warning-card__logo">R</div><strong>⚠ ${state.last?.result.score||0}/100 RISK</strong><h3>Pause before you act.</h3><p>${state.last?.result.verdict==='safe'?'No strong scam pattern was detected. Still verify independently.':'Do not reply, pay, share OTP/PIN/password, or open the link. Verify through an official channel.'}</p><small>Generated locally by Rakshak</small></div><button class="primary-btn" id="copyFamilyModal">Copy warning text</button>`; else m.innerHTML=`<span class="section-kicker">INCIDENT REPORT</span><h2>Local incident record</h2><div class="incident"><div><span>Risk score</span><b>${state.last?.result.score}/100</b></div><div><span>Verdict</span><b>${state.last?.result.verdict}</b></div><div><span>Source</span><b>${state.last?.source}</b></div><div><span>Signals</span><b>${state.last?.result.findings.length}</b></div><hr><p>${esc(state.last?.text||'')}</p></div><button class="primary-btn" id="copyIncident">Copy report</button>`; $('#modalBackdrop').hidden=false; $('#modalBackdrop').style.display='grid'; if($('#copyFamilyModal'))$('#copyFamilyModal').onclick=()=>copyText(warningText(),'Warning copied'); if($('#copyIncident'))$('#copyIncident').onclick=()=>copyText(warningText(),'Report copied');}

function closeModal(){const b=$('#modalBackdrop'); if(!b)return; b.hidden=true; b.style.display='none'; $('#modalContent').innerHTML='';}
function wire(){
 ...
 $('#familyBtn').onclick=()=>openModal('family'); $('#incidentBtn').onclick=()=>openModal('incident'); $('#copyBtn').onclick=()=>copyText(warningText(),'Analysis copied'); $('#cardBtn').onclick=()=>openModal('family'); $('#modalClose').onclick=closeModal; $('#modalBackdrop').onclick=e=>{if(e.target===e.currentTarget)closeModal()}; document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!$('#modalBackdrop').hidden)closeModal()});
